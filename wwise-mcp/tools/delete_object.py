"""
wwise_delete_object — ak.wwise.core.object.delete
Delete a Wwise object by path or GUID.
"""
...
TOOL_NAME = "wwise_delete_object"


def wwise_delete_object(object_ref: str, dry_run: bool = False) -> dict:
    """
    Delete a Wwise object.

    Args:
        object_ref: Path or GUID of the object to delete.
    ...
    try:
        with connect() as client:
            ...
            if result is None:
                err = "Delete call returned None — object may be protected."
                write_phase2_log(TOOL_NAME, WAAPI_URI, checks, False, err)
                return {"success": False, "data": None, "error": err}
            checks["execute"] = True
            checks["post_check"] = not object_exists(client, object_ref)

    except Exception as e:
        write_phase2_log(TOOL_NAME, WAAPI_URI, checks, False, str(e))
        return {"success": False, "data": None, "error": str(e)}

    response = {"success": checks["post_check"], "data": {"deleted": object_ref} if checks["post_check"] else None,
