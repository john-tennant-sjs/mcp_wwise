"""
wwise_log_get — ak.wwise.core.log.get
Retrieve log entries from Wwise (general, soundbank, or media channels).
"""

from __future__ import annotations
from .client import connect, write_phase2_log, validate_response, get_mock_response

WAAPI_URI = "ak.wwise.core.log.get"
TOOL_NAME = "wwise_log_get"

VALID_CHANNELS = {
    "general", "soundbankGenerate", "conversion", "copyPlatformSettings",
    "waapi", "projectLoad", "sourceControl", "lua",
}


def wwise_log_get(
    channel: str = "general",
    dry_run: bool = False,
) -> dict:
    """
    Retrieve log entries from Wwise.

    Args:
        channel: Log channel to read. One of: "general", "soundbank", "media",
                 "reservation", "context". Default: "general".
        dry_run: If True, skip WAAPI call and return mock response.
    """
    checks = {"pre_check": False, "execute": False, "post_check": False, "schema_match": False}

    if channel not in VALID_CHANNELS:
        err = f"Invalid channel '{channel}'. Must be one of {sorted(VALID_CHANNELS)}."
        write_phase2_log(TOOL_NAME, WAAPI_URI, checks, False, err)
        return {"success": False, "data": None, "error": err}
    checks["pre_check"] = True

    if dry_run:
        mock = get_mock_response(TOOL_NAME)
        ok, verr = validate_response(TOOL_NAME, mock)
        checks.update({"execute": True, "post_check": True, "schema_match": ok})
        write_phase2_log(TOOL_NAME, WAAPI_URI, checks, ok, verr)
        return mock

    try:
        with connect() as client:
            result = client.call(WAAPI_URI, {"channel": channel})
            if result is None:
                err = "log.get returned None."
                write_phase2_log(TOOL_NAME, WAAPI_URI, checks, False, err)
                return {"success": False, "data": None, "error": err}
            checks["execute"] = True
            items = result.get("items", [])
            checks["post_check"] = isinstance(items, list)

    except Exception as e:
        write_phase2_log(TOOL_NAME, WAAPI_URI, checks, False, str(e))
        return {"success": False, "data": None, "error": str(e)}

    response = {
        "success": checks["post_check"],
        "data": {"items": items, "channel": channel} if checks["post_check"] else None,
        "error": None if checks["post_check"] else "items key missing",
    }
    ok, verr = validate_response(TOOL_NAME, response)
    checks["schema_match"] = ok
    passed = all(checks.values())
    write_phase2_log(TOOL_NAME, WAAPI_URI, checks, passed, verr if not ok else None)
    return response if passed else {**response, "success": False, "error": verr or "schema_match failed"}


def register(mcp) -> None:
    mcp.tool()(wwise_log_get)
