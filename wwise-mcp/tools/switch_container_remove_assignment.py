"""
wwise_switch_container_remove_assignment — ak.wwise.core.switchContainer.removeAssignment
Remove a child-to-state/switch assignment from a Switch Container.
"""

from __future__ import annotations
from .client import connect, write_phase2_log, validate_response, get_mock_response

WAAPI_URI = "ak.wwise.core.switchContainer.removeAssignment"
TOOL_NAME = "wwise_switch_container_remove_assignment"


def wwise_switch_container_remove_assignment(
    child: str,
    state_or_switch: str,
    dry_run: bool = False,
) -> dict:
    """
    Remove the assignment between a Switch Container child and a State or Switch value.

    Args:
        child:           Path or GUID of the child object.
        state_or_switch: Path or GUID of the State or Switch to unassign.
    ...
    try:
        with connect() as client:
            ...
            if result is None:
                err = "switchContainer.removeAssignment returned None."
                write_phase2_log(TOOL_NAME, WAAPI_URI, checks, False, err)
                return {"success": False, "data": None, "error": err}
            checks["execute"] = True
...
def register(mcp) -> None:
    mcp.tool()(wwise_switch_container_remove_assignment)
