"""
wwise_transport_get_state — ak.wwise.core.transport.getState
Get the state of a transport object.
"""

from __future__ import annotations
from .client import connect, write_phase2_log, validate_response, get_mock_response

WAAPI_URI = "ak.wwise.core.transport.getState"
TOOL_NAME = "wwise_transport_get_state"


def wwise_transport_get_state(
    transport: int,
    dry_run: bool = False,
) -> dict:
    """
    Get the playback state of a transport object.
    ...
    try:
        with connect() as client:
            ...
            if result is None:
                err = "transport.getState returned None."
                write_phase2_log(TOOL_NAME, WAAPI_URI, checks, False, err)
                return {"success": False, "data": None, "error": err}
            checks["execute"] = True
            state = result.get("state")
            checks["post_check"] = state is not None
    ...
    response = {
        ...
        "data": {"state": state} if checks["post_check"] else None,
        "error": None if checks["post_check"] else "state not returned",
    }
...
def register(mcp) -> None:
    mcp.tool()(wwise_transport_get_state)
